...
THC_API void THCudaTensor_bernoulli(THCState* state, THCudaTensor *self_, double p)
{
  ...
  float *data = THCudaTensor_data(state, self);
  #ifdef CURAND_PATH
      hipLaunchKernel(HIP_KERNEL_NAME(generate_bernoulli),
                      dim3(NUM_BLOCKS),
                      dim3(BLOCK_SIZE),
                      0,
                      THCState_getCurrentStream(state),
...
THC_API void THCudaTensor_normal(THCState* state, THCudaTensor *self_, double mean, double stdv)
{
  ...
  float *data = THCudaTensor_data(state, self);
  #ifdef CURAND_PATH
      hipLaunchKernel(HIP_KERNEL_NAME(generate_normal),
                      dim3(NUM_BLOCKS),
                      dim3(BLOCK_SIZE),
                      0,
                      THCState_getCurrentStream(state),
...
__global__
void sampleMultinomialOnce(hipLaunchParm lp,
                           float* dest,
                           long distributions,
                           int categories,
                           float* dist)
...
__global__
void sampleMultinomialWithReplacement(hipLaunchParm lp,
                                      curandStateMtgp32* state,
                                      int totalSamples,
                                      float* dest,
                                      long distributions,
...
__global__
void sampleMultinomialWithoutReplacement(hipLaunchParm lp,
                                         curandStateMtgp32* state,
                                         int totalSamples,
                                         int sample,
                                         float* dest,
