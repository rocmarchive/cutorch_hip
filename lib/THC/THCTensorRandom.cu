...
#ifdef CURAND_PATH
__global__
void set_rngstate_kernel(hipLaunchParm lp, curandStateMtgp32 *state, mtgp32_kernel_params *kernel)
{
...
#ifdef CURAND_PATH
  hipLaunchKernelV2(HIP_KERNEL_NAME(set_rngstate_kernel),
                  dim3(1),
                  dim3(MAX_NUM_BLOCKS),
                  0,
...
__global__                                                                           \
void NAME(hipLaunchParm  lp, curandStateMtgp32 *state, int size, float *result, ARG1)\
{                                                                                    \
...
__global__                                                                                  \
void NAME(hipLaunchParm lp, curandStateMtgp32 *state, int size, float *result, ARG1, ARG2)  \
{                                                                                           \
  int idx = hipBlockIdx_x * BLOCK_SIZE + hipThreadIdx_x;                                    \
  int rounded_size = THCCeilDiv(size, BLOCK_SIZE) * BLOCK_SIZE;                             \
  for (int i = idx; i < rounded_size; i += BLOCK_SIZE * MAX_NUM_BLOCKS) {                   \
...
__global__
void generate_log_normal(hipLaunchParm lp,
...
THC_API void THCudaTensor_uniform(THCState* state, THCudaTensor *self_, double a, double b)
{
  ...
  #ifdef CURAND_PATH
      hipLaunchKernelV2(HIP_KERNEL_NAME(generate_uniform),
                      dim3(NUM_BLOCKS),
                      dim3(BLOCK_SIZE),
                      0,
                      THCState_getCurrentStream(state),
...
THC_API void THCudaTensor_bernoulli(THCState* state, THCudaTensor *self_, double p)
{
  ...
  float *data = THCudaTensor_data(state, self);
  #ifdef CURAND_PATH
      hipLaunchKernelV2(HIP_KERNEL_NAME(generate_bernoulli),
                      dim3(NUM_BLOCKS),
                      dim3(BLOCK_SIZE),
                      0,
                      THCState_getCurrentStream(state),
...
THC_API void THCudaTensor_normal(THCState* state, THCudaTensor *self_, double mean, double stdv)
{
  ...
  float *data = THCudaTensor_data(state, self);
  #ifdef CURAND_PATH
      hipLaunchKernelV2(HIP_KERNEL_NAME(generate_normal),
                      dim3(NUM_BLOCKS),
                      dim3(BLOCK_SIZE),
                      0,
                      THCState_getCurrentStream(state),
...
THC_API void THCudaTensor_logNormal(THCState* state, THCudaTensor *self_, double mean, double stdv)
{
  ...
  float *data = THCudaTensor_data(state, self);
  #ifdef CURAND_PATH
  hipLaunchKernelV2(HIP_KERNEL_NAME(generate_log_normal), dim3(NUM_BLOCKS), dim3(BLOCK_SIZE), 0, THCState_getCurrentStream(state),
...
THC_API void THCudaTensor_geometric(THCState* state, THCudaTensor *self_, double p)
{
  ...
  float *data = THCudaTensor_data(state, self);
  #ifdef CURAND_PATH
  hipLaunchKernelV2(HIP_KERNEL_NAME(generate_geometric), dim3(NUM_BLOCKS), dim3(BLOCK_SIZE), 0, THCState_getCurrentStream(state),
...
THC_API void THCudaTensor_exponential(THCState* state, THCudaTensor *self_, double lambda)
{
  ...
  float *data = THCudaTensor_data(state, self);
  #ifdef CURAND_PATH
  hipLaunchKernelV2(HIP_KERNEL_NAME(generate_exponential), dim3(NUM_BLOCKS), dim3(BLOCK_SIZE), 0, THCState_getCurrentStream(state),
...
THC_API void THCudaTensor_cauchy(THCState* state, THCudaTensor *self_, double median, double sigma)
{
  ...
  float *data = THCudaTensor_data(state, self);
  #ifdef CURAND_PATH
  hipLaunchKernelV2(HIP_KERNEL_NAME(generate_cauchy), dim3(NUM_BLOCKS), dim3(BLOCK_SIZE), 0, THCState_getCurrentStream(state),
...
__global__
void renormRowsL1(hipLaunchParm lp, float* dist, long rows, long cols)
{
  //HIP_DYNAMIC_SHARED(float, smem)
  __shared__ float smem[1024]; // TODO: temporary workaround.
  ...
  for (long row = hipBlockIdx_x; row < rows; row += hipGridDim_x) {
    ...
    sum = reduceBlock(
        static_cast<float*>(static_cast<void*>(smem)),
        hipBlockDim_x,
        sum,
...
void THCudaTensor_renormRows(struct THCState* state, THCudaTensor* t)
{
  ...
  hipLaunchKernelV2(HIP_KERNEL_NAME(renormRowsL1),
                  dim3(grid),
                  dim3(block),
...
__global__
void sampleMultinomialWithoutReplacement(
    hipLaunchParm lp,
    curandStateMtgp32* state,
    int totalSamples,
    int sample,
    ...
    int with_replacement)
{
  ...
  if (n_sample == 1) {
    ...
    hipLaunchKernelV2(
        HIP_KERNEL_NAME(sampleMultinomialOnce),
        dim3(grid),
        dim3(block),
        block.x * sizeof(float),
        THCState_getCurrentStream(state),
    ...
    if (with_replacement) {
      ...
      hipLaunchKernelV2(
          HIP_KERNEL_NAME(sampleMultinomialWithReplacement),
          dim3(grid),
          dim3(block),
          0,
          THCState_getCurrentStream(state),
      ...
      for (int sample = 0; sample < n_sample; ++sample) {
        ...
        hipLaunchKernelV2(
            HIP_KERNEL_NAME(sampleMultinomialWithoutReplacement),
            dim3(grid),
            dim3(block),
            0,
            THCState_getCurrentStream(state),
