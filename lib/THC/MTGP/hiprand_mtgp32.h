...
static inline double erfcinvd(double x) __attribute__((hc, cpu)) {
  return do_erfcinv (x, true);
}

static inline float erfcinvf(float x) __attribute__((hc, cpu)) {
  return do_erfcinv (x, false);
}

static inline float _hiprand_normal_icdf(unsigned int x) __attribute__((hc, cpu)) {
