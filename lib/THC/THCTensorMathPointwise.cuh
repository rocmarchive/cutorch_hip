...
struct TensorSignOp<unsigned char> {
  __device__ __forceinline__
  void operator()(unsigned char* out, unsigned char* in)
  {
...
struct TensorAddOp {
  ...
  void operator()(T* out, T* in1, T* in2) { *out = *in1 + *in2; }
    ...
    struct TensorAddOp<half> {
      ...
      __device__ __forceinline__
      void operator()(half* out, half* in1, half* in2)
      {
        #ifdef CUDA_HALF_INSTRUCTIONS
          *out = __hadd(*in1, *in2);
        #else
...
struct TensorCAddOp {
  ...
  __device__ __forceinline__
  void operator()(T* out, T* in) { *out += val * *in; }

  __device__ __forceinline__
  void operator()(T* out, T* in1, T* in2) { *out = *in1 + val * *in2; }
...
struct TensorPowOp {
  ...
  __device__ __forceinline__
  void operator()(T* out, T* in) { *out = powf((float) *in, (float) val); }

  __device__ __forceinline__
  void operator()(T* v) { *v = powf((float) *v, (float) val); }
...
struct TensorPowOp<double> {
  ...
  __device__ __forceinline__
  void operator()(double* out, double* in) { *out = pow(*in, val); }

  __device__ __forceinline__
  void operator()(double* v) { *v = pow(*v, val); }
...
struct TensorCPowOp {
  __device__ __forceinline__
  void operator()(T* out, T* in) { *out = powf((float) *out, (float) *in); }

  __device__ __forceinline__
  void operator()(T* out, T* in1, T* in2)
  {
...
struct TensorCPowOp<double> {
  __device__ __forceinline__
  void operator()(double* out, double* in) { *out = pow(*out, *in); }

  __device__ __forceinline__
  void operator()(double* out, double* in1, double* in2)
  {
      *out = pow(*in1, *in2);
  }
};
