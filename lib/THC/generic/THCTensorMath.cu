...
#define HANDLE_CASE(DIMS) \
  hipLaunchKernelGGL((CatArrayBatchedCopy<real, unsigned int, DIMS>), applyGrid, applyBlock, 0, THCState_getCurrentStream(state), data, d_inputs, param, cat_dimension, param.devOutputStride);
    ...
    for (i = 0; i < numInputs; i += CAT_ARRAY_BATCH_SIZE) {
      ...
      THCudaCheck(hipMemcpy(d_inputs, stackInputs, j * sizeof(CatArrInputTensor<real, unsigned int>), hipMemcpyHostToDevice));
...
void THCTensor_(diag)(THCState *state, THCTensor *self_, THCTensor *src_, long k){
  ...
  if (nDimension == 2) {
    ...
    hipLaunchKernelGGL((THCTensor_copyFromDiagonal<real>), grid, threads, 0, THCState_getCurrentStream(state),
    ...
    hipLaunchKernelGGL((THCTensor_copyToDiagonal<real>), grid, threads, 0, THCState_getCurrentStream(state), 
