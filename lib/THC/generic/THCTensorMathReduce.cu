    ...
    long dim)
{
  ...
  THCTensor_(div)(
    ...
    self,
    self,
    ScalarConvert<long, real>::to(THCTensor_(size)(state, src, dim)));
}
        ...
        real maxnorm)
    {
      ...
      THArgCheck(
        dimension >= 0 && dimension < THCTensor_(nDimension)(state, src),
        3,
        "invalid dimension");
      THArgCheck(
        THCNumerics<real>::gt(value, ScalarConvert<int, real>::to(0)),
        2,
        "non-positive-norm not supported");
      THArgCheck(
        THCTensor_(nDimension)(state, src) > 1, 1, "need at least 2 dimensions");
        ...
        int flag)
    {
      ...
      if (dimension == THCTensor_(nDimension)(state, src) - 1) {
        ...
        THCTensor_varOuterDim<THCTensor, real, true>(
            state,
            self,
            src,
            dimension,
        ...
        int flag)
    {
      ...
      if (dimension == THCTensor_(nDimension)(state, src) - 1) {
        ...
        THCTensor_varOuterDim<THCTensor, real, false>(
            state,
            self,
            src,
            dimension,
        ...
        long dimension)
    {
      ...
      if (THCNumerics<real>::eq(value, ScalarConvert<float, real>::to(0.0))) {
        THC_reduceDim(
            ...
            ReduceAdd<real, real>{},
            ScalarConvert<float, real>::to(0.0),
            dimension);
      }
    ...
    long dimension)
{
  ...
  bolt::amp::pair<typename TensorUtils<THCTensor>::DataType, long> init{
