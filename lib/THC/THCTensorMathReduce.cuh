...
struct ReduceAdd {
  ...
  AccT operator()(AccT a, InT b) const { return a + (AccT) b; }
    ...
    struct ReduceAdd<half, float> {
      __device__
      float operator()(float a, half b) const
      {
        #if defined(__HIP_PLATFORM_HCC__)
          return a + b;
        #else
...
void THCTensor_kernel_renorm(
    Real *data, Real value, ptrdiff_t size, Real maxnorm)
{
...
void THCTensor_kernel_varInnermostDim(
    Real *tgt, Real *src_, unsigned num_rows, unsigned row_size)
{
...
void THCTensor_varInnermostDim(
    THCState *state, TensorTypeK *tgt, TensorTypeK *src, int flag)
{
  unsigned ndim = TensorUtils<TensorTypeK>::getDims(state, src);
...
template<
    typename TensorTypeK, typename TensorTypeIndex, typename BinaryFunction>
