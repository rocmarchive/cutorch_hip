		...
		concurrency::parallel_for_each( av, tileK2,
				...
				] ( concurrency::tiled_index< kernel2_WgSize > t_idx ) restrict(amp)
		  {
				...
				for( int offset = 1; offset < wgSize; offset *= 2 )
				{
					...
					if (locId >= offset)
					{
						sum = binary_op(sum, lds[locId - offset]);
					}
					t_idx.barrier.wait();
					lds[ locId ] = sum;
				}
...
template<typename InputIterator,
		 ...
		 typename std::enable_if<sizeof(typename std::iterator_traits<InputIterator>::value_type) % sizeof(int) == 0 &&
		             			 sizeof(typename std::iterator_traits<OutputIterator>::value_type) % sizeof(int) == 0 &&
					             sizeof(T) % sizeof(int) == 0>::type* = nullptr>
static
inline
OutputIterator exclusive_scan(
    InputIterator first,
    ...
    T init)
{
	 return bolt::amp::exclusive_scan(
           control::getDefault( ),
		   first,
		   last,
		   result,
...
OutputIterator exclusive_scan(
		...
		T init)
{
	return detail::serial::scan(first, last, result, init, false);
};
...
OutputIterator exclusive_scan(
        ...
        BinaryFunction binary_op)
{
    return detail::serial::scan(first, last, result, init, false, binary_op);
};
